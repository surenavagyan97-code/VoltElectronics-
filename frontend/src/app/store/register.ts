...
@Component({
  ...
  template: `
    <div style="padding: 64px 32px; max-width: 400px; margin: 0 auto;">
      ...
      <p class="text-muted" style="margin-bottom: 24px;">Track orders and check out faster.</p>
      <form class="col" style="gap: 14px;" (ngSubmit)="submit()">
        ...
        <div class="field"><label>Password <span class="text-muted">(8+ characters)</span></label>
      ...
      <div class="hr"></div>
      <p class="text-muted" style="font-size: 14px;">Already have an account? <a routerLink="/login">Sign in</a></p>
    </div>
  `,
})
