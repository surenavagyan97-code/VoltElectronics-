...
/// </summary>
internal sealed class AmeriaVposGateway(
    HttpClient http,
    IOptions<PaymentsOptions> options,
    ILogger<AmeriaVposGateway> logger) : IPaymentGateway
{
    private readonly PaymentsOptions _payments = options.Value;
    private AmeriaVposOptions Opts => _payments.Ameria;
    ...
    public async Task<PaymentInitResult> InitPaymentAsync(PaymentInitRequest request, CancellationToken cancellationToken = default)
    {
        ...
        var body = new
        {
            ...
            OrderID = Opts.OrderIdOffset + request.OrderId,
            Amount = request.Amount,
            Currency = request.Currency,
            Description = request.Description,
            BackURL = _payments.CallbackUrl(),
        ...
        try
        {
            var httpRes = await http.PostAsJsonAsync($"{Opts.BaseUrl}/api/VPOS/InitPayment", body, cancellationToken);
            httpRes.EnsureSuccessStatusCode();
            res = await httpRes.Content.ReadFromJsonAsync<InitPaymentResponse>(cancellationToken);
        }
        catch (Exception ex)
        {
        ...
        var payUrl = $"{Opts.BaseUrl}/Payments/Pay?id={Uri.EscapeDataString(res.PaymentId)}&lang={Opts.Language}";
        return PaymentInitResult.Ok(res.PaymentId, payUrl);
    }

    public async Task<PaymentVerifyResult> VerifyCallbackAsync(IReadOnlyDictionary<string, string?> query, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var httpRes = await http.PostAsJsonAsync($"{Opts.BaseUrl}/api/VPOS/GetPaymentDetails",
                new { PaymentID = paymentId, Username = Opts.Username, Password = Opts.Password }, cancellationToken);
            httpRes.EnsureSuccessStatusCode();
            details = await httpRes.Content.ReadFromJsonAsync<PaymentDetailsResponse>(cancellationToken);
        }
        catch (Exception ex)
        {
