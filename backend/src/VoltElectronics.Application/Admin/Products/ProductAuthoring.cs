...
internal static class ProductAuthoring
{
    ...
    public static IEnumerable<(string Lang, string? Name, string? Description)> ToTranslationPairs(
        this List<ProductTranslationDto>? translations) =>
        (translations ?? []).Select(t => (t.Lang, t.Name, t.Description));
}
